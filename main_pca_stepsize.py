# coding: utf-8
# Script for evaluating the performance of Riemannian diffusion algorithm with different step sizes
#
# Reference: 
...
# parameter settings
step_sizes = [5e-2, 7e-2, 9e-2]  # diffusion
step_size_fm = 8e-1 # diffusion

# metrics
MSD_dif_all = np.zeros([np.size(step_sizes), N_samples])
...
for num_step_size in range(np.size(step_sizes)):
    step_size = step_sizes[num_step_size]
    ...
    # mean errros
    MSD_dif_all[num_step_size, :] = np.mean(MSD_dif, axis=0)

# draw figures
fig = plt.figure(figsize = (4.8, 3.6), dpi = 150)
colors = ["#82B0D2", "#FFBE7A", "#FA7F6F", "#2F7FC1"]
for num_step_size in range(np.size(step_sizes)):
    plt.plot(10 * np.log10(MSD_dif_all[num_step_size, :]), label='$\mu = ' + str(step_sizes[num_step_size]) + '$', color=colors[num_step_size])
plt.xlabel("iteration")
plt.ylabel("MSD (dB)")
plt.legend()
plt.tight_layout()
plt.grid(True)
plt.savefig("figures/MSD_pca_stepsize.pdf", bbox_inches='tight')
plt.show()
